...
def paraphrase_prompt(input_text, sys_msg=system_message):
    """
...
def validate_datetime():
    ...
    lot = data.get("lot")
    date_input = data.get("date")  # <-- replaced "day" with "date"
    time_input = data.get("time")

    # Require lot, date, and time
    if not lot or not date_input or not time_input:
        return jsonify(error="lot, date, and time are required."), 400

    if lot not in parsed_data:
        return jsonify(error=f"Unknown lot: {lot}"), 400

    # ---------------------------------------------------------------
    # 1) Convert the "date" input into a day-of-week abbreviation
    # ---------------------------------------------------------------
    try:
        # Support 'today'/'tomorrow' or parse MM-DD-YYYY
        if date_input.lower() == "today":
        ...
        return jsonify(
            error="Invalid date format. Please use MM-DD-YYYY, 'today', or 'tomorrow'."
        ), 400

    # Extract abbreviated weekday, e.g. "mon", "tue", etc.
    day_input = parsed_date.strftime("%a").lower()

    # ---------------------------------------------------------------
    # 2) *Unchanged* day-mapping logic
    #    (Preserving your for-loop and 'Always' check exactly)
    # ---------------------------------------------------------------
    day_map = {
    ...
    if not found_day:
        ...
        if day_input in lot_days:
            ...
            return jsonify(
                error="Could not interpret day. Please specify 'Weekdays' or 'Weekends'.",
                day_options=lot_days
            ), 400

    if found_day not in parsed_data[lot]['Permissions']:
        return jsonify(
            error=f"No permission data available for '{found_day}' in lot {lot}. "
                  f"Available options: {lot_days}"
        ), 400

    # ---------------------------------------------------------------
    # 3) Time parsing + interval matching (untouched)
    # ---------------------------------------------------------------
    ...
    if not matched_interval:
        ...
        payload = {
            "alert_message": "A time gap was discovered!.",
